...
class EnhancedGameReviewAnalyzer:
    ...
    def fetch_reviews(self, app_id, store='appstore', country='us', language='en', max_reviews=10000):
	    ...
	    """
	    all_reviews = []
	    all_review_ids = set()  # Track IDs to avoid duplicates across regions
	    ...
	    if store.lower() == 'appstore':
	        # Multi-region approach for App Store to overcome the 678 review limit
	        # Define regions to try - larger markets are likely to have more reviews
	        countries = ['us', 'gb', 'ca', 'au', 'de', 'fr', 'jp', 'es', 'it', 'cn', 'kr', 'br', 'mx', 'ru', 'in']
	        
	        progress_text.text(f"Fetching App Store reviews from multiple regions...")
	        
	        for country_code in countries:
	            if len(all_reviews) >= max_reviews:
	                break
	                
	            progress_text.text(f"Fetching App Store reviews from {country_code.upper()}...")
	            region_reviews = []
	            ...
	            while page <= max_pages and len(region_reviews) + len(all_reviews) < max_reviews:
	                # Use different sort orders to get more diverse reviews
	                sort_orders = ['mostRecent', 'mostHelpful', 'mostFavorable', 'leastFavorable']
	                sort_order = sort_orders[min((page-1) % len(sort_orders), len(sort_orders)-1)]
	                
	                # Update progress with metrics
	                elapsed = time.time() - start_time
	                rate = len(all_reviews) / elapsed if elapsed > 0 else 0
	                progress_text.text(f"Fetching {country_code.upper()} App Store: page {page}/{max_pages}, sort: {sort_order} ({len(all_reviews) + len(region_reviews)} reviews, {rate:.1f}/sec)")
	                
	                # Run garbage collection periodically
	                if (len(all_reviews) + len(region_reviews)) % 1000 == 0 and len(all_reviews) + len(region_reviews) > 0:
	                    gc.collect()
	                
	                url = f"https://itunes.apple.com/{country_code}/rss/customerreviews/page={page}/id={app_id}/sortBy={sort_order}/json"
	                ...
	                try:
	                    ...
	                    if response.status_code == 200:
	                        ...
	                        if len(entries) > 1:
	                            ...
	                            for review in reviews:
	                                try:
	                                    ...
	                                    review_data = {
	                                        ...
	                                        'store': f'App Store ({country_code.upper()})'
	                                    }
	                                    
	                                    region_reviews.append(review_data)
	                            ...
	                            if len(reviews) == 0:
	                                ...
	                                if empty_page_count >= 3:
	                                    break  # Move to next region after 3 empty pages
	                            else:
	                                empty_page_count = 0
	                        else:
	                            # No more reviews in this region with this sort
	                            empty_page_count += 1
	                            if empty_page_count >= 3:
	                                break  # Move to next region after 3 empty pages
	                        ...
	                        # Other errors, try next region
	                        break
	                except requests.RequestException as e:
	                    # Network errors, try next region
	                    break
	            
	            # Add this region's reviews to the total
	            all_reviews.extend(region_reviews)
	            progress_text.text(f"Got {len(region_reviews)} unique reviews from {country_code.upper()} App Store. Total so far: {len(all_reviews)}")
	            time.sleep(0.5)  # Short pause between regions
	    
	    elif store.lower() == 'googleplay' and GOOGLE_PLAY_SCRAPER_AVAILABLE:
	        progress_text.text(f"Fetching Google Play reviews...")
	        
	        # Enhanced Google Play review collection
	        ...
	        while len(all_reviews) < max_reviews and current_sort_index < len(sort_methods):
	            ...
	            batch_count += 1
	            if batch_count > 1:
	                batch_size = min(200, max_reviews - len(all_reviews))  # Google Play can handle larger batches
	            else:
	            ...
	            try:
	                result, new_token = gplay_reviews(
	                    app_id,
	                    lang=language,
	                    country=country,
	                    sort=current_sort,
	                ...
	                if result:
	                    ...
	                    progress_text.text(f"Added {len(new_reviews)} new Google Play reviews. Total: {len(all_reviews)}")
	                    
	                    # Update continuation token
	                    continuation_token = new_token
	                    
	                    # If we got no new reviews or no token, try a different sort method
	                    if not new_reviews or not continuation_token:
	                        current_sort_index += 1
	                        continuation_token = None
	                
	                else:
	                    # No results, try next sort method
	                    current_sort_index += 1
	                    continuation_token = None
	                ...
	                # After several retries, try a different sort method
	                if retry_count >= 3:
	                    current_sort_index += 1
	                    continuation_token = None
	                    retry_count = 0
	    ...
	    progress_text.text(f"Completed! Fetched {len(all_reviews)} reviews in {total_time:.1f} seconds (avg: {avg_rate:.1f} reviews/sec)")
